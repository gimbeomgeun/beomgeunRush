...
//https://www.youtube.com/watch?v=0qqzGkbLbpM&t=959s

ZeroCollider::ZeroCollider(RigidBody2D* b1, RigidBody2D* b2)
{
    if (b1 < b2)
    {
        body1 = b1;
    ...
    numContacts = ZeroColliderManager::Collide(contacts, body1, body2);

    friction = sqrtf(body1->friction * body2->friction);
}

void ZeroCollider::Update(Contact* newContacts, int numNewContacts)
{
    ...
    for (int i = 0; i < numNewContacts; ++i)
    {
        ...
        if (k > -1)
        {
            ...
            *c = *cNew;
            if (ZeroColliderManager::warmStarting)
            {
                c->Pn = cOld->Pn;
                c->Pt = cOld->Pt;
                c->Pnb = cOld->Pnb;
...
void ZeroCollider::PreStep(float inv_dt)
{
    if(body1->isTrigger || body2->isTrigger)
        return;

    const float k_allowedPenetration = 0.01f;
    float k_biasFactor = ZeroColliderManager::positionCorrection ? 0.2f : 0.0f;

    for (int i = 0; i < numContacts; ++i)
    {
        Contact* c = contacts + i;

        Vec2 r1 = c->position - body1->GetOwner()->transform->GetWorldPos();
        Vec2 r2 = c->position - body2->GetOwner()->transform->GetWorldPos();
        ...
        if (ZeroColliderManager::accumulateImpulses)
        {
...
void ZeroCollider::ApplyImpulse()
{
    if(body1->isTrigger || body2->isTrigger)
        return;

    RigidBody2D* b1 = body1;
    RigidBody2D* b2 = body2;

    for (int i = 0; i < numContacts; ++i)
    {
        Contact* c = contacts + i;
        c->r1 = c->position - b1->GetOwner()->transform->GetWorldPos();
        c->r2 = c->position - b2->GetOwner()->transform->GetWorldPos();
